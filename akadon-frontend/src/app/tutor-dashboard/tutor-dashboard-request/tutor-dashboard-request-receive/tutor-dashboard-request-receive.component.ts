...
import { TutorRequest } from 'src/app/models/tutor-request.model';
...
export class TutorDashboardRequestReceiveComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    var filter =
      ...
      " and TR.requestType= N'Từ học viên'";
  ...
  submitEditForm() {
    ...
    studentRequest.status = 'Đã được nhận, vui lòng kiểm tra';
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.tutorRequestService.getById(id).subscribe((res) => {
          ...
          studentRequest.status = 'Chưa có gia sư nào yêu cầu';
          this.studentRequestService
            .updateStudentRequest(studentRequest)
            .subscribe();
          var notification: Notifications = {};
