...
import { StudentRequest } from 'src/app/models/student-request.model';
...
@Component({
  selector: 'app-student-dashboard-send',
  templateUrl: './student-dashboard-send.component.html',
  styleUrls: ['./student-dashboard-send.component.css'],
})

export class StudentDashboardSendComponent implements OnInit {
  constructor(
  ...
  ngOnInit(): void {
    ...
    var filter =
      ...
      " and (status=N'Chưa có gia sư nào yêu cầu' or status=N'Đã gửi, chờ gia sư đồng ý')";
  ...
  deleteRequest(event: any) {
    var id = event.currentTarget.id;
    id = id.toString().slice(7, id.length);
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.studentRequestService.deleteStudentRequest(id).subscribe((res) => {
          if (res === 'Thành công') {
            Swal.fire('Xóa thành công!', 'Yêu cầu này đã được xóa', 'success');
            this.studentRequestService
              .filter(this.mainQuery, 1, this.pageSize)
              .subscribe((res) => {
                this.studentRequest = res;
                this.length = res.length;
              });
            ...
            var result: string = res as string;
            Swal.fire({
              icon: 'error',
              title: 'Xóa thất bại',
              text: result,
